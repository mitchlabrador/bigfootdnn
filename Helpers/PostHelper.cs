...
namespace BigfootDNN.Helpers
{
    ...
    public class PostHelper
    {
        ...
        /// </summary>
        public PostHelper(bool inTestingMode = false)
        {
        ...
        /// </summary>
        public PostHelper(HttpRequestBase request)
        {
        ...
        /// </summary>
        public PostHelper(HttpContextBase context, NameValueCollection manualValues, bool inTestingMode)
        {
            this.Context = context;
            this.Request = context.Request;
            this.ManualValues = manualValues;
            this.InTestingMode = context == null || inTestingMode;
        }
        ...
        public NameValueCollection RequestValues
        {
            get
            {
                // Try to get the values from the request cache
                if (!InTestingMode && Context.Items.Contains(cacheKey))
                    return (NameValueCollection)Context.Items[cacheKey];
                ...
                {
                    values.Add(ManualValues);                	
                }
                
                // When in testing do not inspect the context
                if (InTestingMode) return values;
        ...
        /// Returns a value for a certain parameter typed as an date 
        /// </summary>
        public DateTime GetDateOrDefault(string key, DateTimeFormatInfo format)
        {
            var result = new DateTime();
            DateTime value;
            if (RequestValue(key) != "" && DateTime.TryParse(GetValue(key), format, DateTimeStyles.None, out value))
            {
                result = value;
            }
            return result;
